            ...
            <div class="col-sm-12 col-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <div class="card-title">Danh sách giảm giá</div>
                        <div class="d-flex justify-content-start">
                            <a href="{{ route('discounts.create') }}" class="btn btn-sm rounded-pill btn-primary d-flex align-items-center me-2">
                                <i class="bi bi-plus-circle me-2"></i> Thêm Giảm Giá
                            </a>
                            <a href="{{ route('admin.catalogueList') }}" class="btn btn-sm rounded-pill btn-primary d-flex align-items-center">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @else
                                            Giảm giá theo tiền
                                            @endif</td> <!-- Loại giảm giá (percentage/fixed) -->
                                        <td>
                                            @if($discount->type == 'percentage')
                                        ...
                                        <td>{{ \Carbon\Carbon::parse($discount->start_date)->format('d-m-Y H:i') }}</td> <!-- Ngày bắt đầu -->
                                        <td>{{ \Carbon\Carbon::parse($discount->end_date)->format('d-m-Y H:i') }}</td> <!-- Ngày kết thúc -->
                                        <td>{{ \Carbon\Carbon::parse($discount->created_at)->format('d-m-Y H:i') }}</td> <!-- Ngày Tạo -->
                                        <td>{{ \Carbon\Carbon::parse($discount->updated_at)->format('d-m-Y H:i') }}</td> <!-- Ngày sửa -->
                                        <td>
